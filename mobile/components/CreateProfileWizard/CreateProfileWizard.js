...
async function uploadProfile(state, ids) {
  ...
  const body = [...Array(6).keys()].reduce(
    (acc, elem) => {acc.append(`photo${elem}`, ids[elem] ? ids[elem] : 11); return acc;}, new FormData());
  body.append('bio', state.bio);
  console.log(body);
  const result = await APICall.PostAuth(url,  {'Content-Type': 'multipart/form-data'}, body);
